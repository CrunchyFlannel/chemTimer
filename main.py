...
def chemTimer(currentHour, currentMinute, delta):
	'''the percent sign means mod() so in your case do mod(currentMinute + delta, 60) 
	if this does not work try flipping the 60 and the currentMinute'''
...
'''This calls the function, try any inputs you would like it works everytime. Also the problem states you may have problems 
in matlab if the delta input is negative. If you do have problems just remember you can multiply the inputs by -1 to flip the sign.
